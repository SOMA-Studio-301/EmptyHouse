...
/// <summary>
/// 자물쇠 면 — 잠긴 문의 자물쇠 상호작용부(LockInteractable 의 문 결합판, WardrobeEntrance 위치).
/// 판정·프롬프트·소음만 갖고 상태를 소유하지 않는다 — 확정(해정·열쇠 소멸)은 문 루트의 서버 RPC(M7 요구 1).
/// 독립형 LockInteractable(수동 배치 맵)과 별개 — 절차 생성 문은 이 면을 쓴다.
/// </summary>
public sealed class DoorLockFace : SingleClickInteractableBase
{
    [Header("Door")]
    [SerializeField] private DoorInteractable door; // 상태 단일 소스인 문 루트(프리팹 내부 부모 참조 — 자기완결 허용 의존)
    ...
    /// <summary>
    /// 프롬프트를 반환한다: 문 개방·비잠금 → 미표시 / 손에 쌍 열쇠(door.PairId) → 활성 / 그 외 → 비활성.
    ...
    /// <summary>해정 시도 효과 — 소음 1회 발행 후 문 루트에 해정+개방을 요청한다. 열쇠 소멸은 서버 승인 후(루트 소관).</summary>
    ...
    protected override void OnActivate(PlayerInteractor interactor)
    {
        ...
        if (door.IsOpen || !door.IsLocked || !HasMatchingKeyInHand(interactor))
        {
            return;
        }
