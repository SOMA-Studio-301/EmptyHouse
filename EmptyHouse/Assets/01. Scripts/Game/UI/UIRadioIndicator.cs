...
public class UIRadioIndicator : MonoBehaviour
{
    ...
    [Header("Sprites")]
    [SerializeField] private Sprite idleSprite; // 보유 + 대기 스프라이트(무전기 OFF)
    [SerializeField] private Sprite transmittingSprite; // 보유 + 송신 중 스프라이트(무전기 ON)
    ...
    /// <summary>표시 상태를 아이콘에 반영한다. 미보유면 숨기고, 그 외에는 대기/송신 스프라이트를 맞바꾼다.</summary>
    ...
    private void Render(RadioHudState state)
    {
        ...
        if (state == RadioHudState.None) return;

        iconImage.sprite = state == RadioHudState.Transmitting ? transmittingSprite : idleSprite;
    }
}
