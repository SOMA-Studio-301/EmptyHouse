...
namespace EmptyHouse.MapGen.Editor
{
    ...
    public static class MapGenSceneBuilder
    {
            ...
            for (int e = 0; e < blueprint.Edges.Count; e++)
            {
                ...
                if (edge.RoomB < 0)
                {
                    ...
                    if (FindTemplate(templates, blueprint.Rooms[edge.RoomA].TemplateId).IsCorridor
                        && !TryOpenSealedHalfMouth(blueprint, templates, edge, e, roomInstances, doorsRoot.transform, mapRoot.transform.position, minX, minY))
                    {
        ...
        /// Seal 대신 그 방 벽을 절단해 전폭 개구로 만든다 — 같은 두 공간이라 그래프 연결성 불변(런타임 동일 규칙).
        ...
        private static bool TryOpenSealedHalfMouth(MapBlueprint blueprint, IReadOnlyList<RoomTemplateDef> templates, BlueprintEdge sealedEdge, int edgeIndex, GameObject[] roomInstances, Transform doorsRoot, Vector3 mapOrigin, int minX, int minY)
        {
            ...
            Vector3 profileCenter = boundaryAlongX
                ...
                : new Vector3(gateCenter.x, mapOrigin.y + 3f, cellCenterAxis);
            var profile = new Bounds(profileCenter, boundaryAlongX ? new Vector3(4f, 6f, 1.6f) : new Vector3(1.6f, 6f, 4f));
            CoverOpeningSlits(cut, profile, boundaryAlongX, doorsRoot, mapOrigin.y, edgeIndex);
            return true;
        }
        ...
        private static void PlaceCornerColumns(MapBlueprint blueprint, IReadOnlyList<RoomTemplateDef> templates, Transform columnsRoot, Vector3 mapOrigin, int minX, int minY)
        {
            ...
            // 격자점별 벽선 판정 — L자(직각 2개) + 잼(벽선 1개가 방↔방 통로로 끊기는 점) 채택
